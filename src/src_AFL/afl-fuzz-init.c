  ...
  while (q) {
    ...
    if (!strncmp(rsl, CASE_PREFIX, 3) &&
      ...
      nfn = alloc_printf("%s/queue/id:%06u,time:0,orig:%s", afl->out_dir, id,
                         use_name);
      /* Create automaton file as well */
      nfn_aut_orig = alloc_printf("%s.aut", q->fname);
      nfn_aut = alloc_printf("%s/queue/id:%06u,time:0,orig:%s.aut", afl->out_dir, id,
                         use_name);
    ...
    ck_free(q->fname);
    q->fname = nfn;
    // Update the size of the pivoted queue entry
    FILE* fp = fopen(nfn, "r");
...
state *create_pda(u8* automaton_file) {
    ...
    // parsed_json = json_object_from_file("./gramfuzz/php_gnf_processed_full.json");
    ...
    pda = (state *)calloc(atoi(json_object_get_string(source_obj)) + 1, sizeof(state));

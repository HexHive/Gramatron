/*
   ...
   Modifications Copyright (c) 2021 HexHive Group, Prashast Srivastava, Mathias Payer
  ...
  LIST_FOREACH(&afl->custom_mutator_list, struct custom_mutator, {
    ...
    if (el->afl_custom_fuzz) {
      ...
      retry_external_pick:
        ...
        // Commenting reading the additional testcase out because we are only interested
        // in the filename so that we can read the corresponding input representation
        // in case the splice mutator is picked
        ...
        size_t mutated_size =
            el->afl_custom_fuzz(el->data, out_buf, len, &mutated_buf, target->fname,
                                target->len, max_seed_size);
        
        // This check is commented out because if the random rec mutator is chosen
        // then it is possible for mutated buf to be empty if no recursive features
        // are present
        ...
        if (mutated_size > SIZE_THRESHOLD) {
           // printf("\nSize threshold reached..ignoring mutant");
           continue ;
        } else if (mutated_size > 0) {
